...
#endif
{
	...
	else if (usrsp->proc_hello_flag) 
	{
	  *eof = 1;
		...
		len=scnprintf(sbuf,len,"Hello .. I got \"%s\"\n",usrsp->proc_hello_value); 
	}
	else
  {
	  *eof = 1;
		len=scnprintf(sbuf,len,"Hello from process %d\n", (int)current->pid);
	}
...
static int my_init (void) 
{
	...
	// proc_mysubdir = proc_mkdir("subdir",proc_mydev);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
  proc_hello = proc_create("hello", 0777, proc_mydev, &proc_fops);
  // proc_hello = proc_create("hello", 0777, proc_mysubdir, &proc_fops);
#else
