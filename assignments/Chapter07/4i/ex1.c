...
If the mutex is of the “normal” or "timed" type, the calling thread is suspended until the mutex is unlocked, thus effectively causing the calling thread to block. 
...
void *fn(void *str)
{
  ...
  printf("%s:Inside fn(): Thread '%lu' Counter value: %d.\n",
	...
	while (i++<ITERS) {
   	...
   	printf("%s:'%lu' Trying to Acquire Mutex:Thread '%lu':Counter value: %d.\n",
