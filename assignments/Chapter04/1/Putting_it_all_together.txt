...
##     ii) Run this command
           dmesg | tail | grep -E "sd.:" | sort -u | sed 's/\[.*.*\] *//'
...
## a. Download latest u-boot
           ...
           Latest=$(grep -E " was released on " release_cycle.html | head -1 | sed 's~</*[lp]i*>~~g')
...
##          ## step #c2: download latest RPi /boot directory
            ## --  download and copy files into the boot partition of the SD card. 
            #
            ## svn checkout https://github.com/raspberrypi/firmware/trunk/boot
            git clone https://github.com/raspberrypi/firmware.git  ## this is a large download
            ## Alternatively, download 
            ## from
            ## --
            ## for Raspberry Pi 4
            ## 1.  https://github.com/raspberrypi/firmware/blob/master/boot/bootcode.bin
            ## 2.  https://github.com/raspberrypi/firmware/blob/master/boot/start4.elf
            sudo cp boot/bootcode.bin /mnt/boot/
            sudo cp boot/start4.elf /mnt/boot/
            ## --
            ## for Raspberry Pi 3
            ## 1.  https://github.com/raspberrypi/firmware/blob/master/boot/bootcode.bin
            ## 2.  https://github.com/raspberrypi/firmware/blob/master/boot/start.elf
            ## 3.  https://github.com/raspberrypi/firmware/blob/master/boot/fixup.dat
            sudo cp boot/bootcode.bin /mnt/boot/
            sudo cp boot/start.elf /mnt/boot/
            sudo cp boot/fixup.dat /mnt/boot/
